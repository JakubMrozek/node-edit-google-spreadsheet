...
var init = function(opts, callback) {
  ...
  auth(opts, function(err, token) {
    ...
    callback(null, spreadsheet, token);
  });
};
...
exports.create = exports.load = function(opts, callback) {
  ...
  init(opts, function (err, spreadsheet, token){
    if (err) return callback(err);
...
exports.list = function(opts, callback) {
  init(opts, function (err, spreadsheet, token){
    var url = spreadsheet.protocol + '://spreadsheets.google.com/feeds/worksheets/' + opts.spreadsheetId + '/private/full?alt=json';
    spreadsheet.request({ url: url }, function(err, result) {
      if (err) return callback(err);
      var entries = result.feed.entry || [];
      var results = [];
      for (var i = 0; i < entries.length; ++i) {
        var title = entries[i].title['$t'];
        var id = entries[i].id['$t'].split('/')[6];
...
Spreadsheet.prototype.receive = function(options, callback) {
  ...
  this.request({
    ...
    callback(null,rows,info, response);
  });
};
